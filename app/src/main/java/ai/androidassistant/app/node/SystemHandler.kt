...
private const val NOTIFICATION_CHANNEL_BASE_ID = "androidassistant.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "AndroidAssistant Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "AndroidAssistant Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "AndroidAssistant Active")
      }
