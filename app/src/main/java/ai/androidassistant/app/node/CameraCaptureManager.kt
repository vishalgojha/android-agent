...
import ai.androidassistant.app.PermissionRequester
...
class CameraCaptureManager(private val context: Context) {
  ...
  suspend fun clip(paramsJson: String?): FilePayload =
    withContext(Dispatchers.Main) {
      ...
      val file = File.createTempFile("android-assistant-clip-", ".mp4")
...
private suspend fun ImageCapture.takeJpegWithExif(executor: Executor): Pair<ByteArray, Int> =
  suspendCancellableCoroutine { cont ->
    val file = File.createTempFile("android-assistant-snap-", ".jpg")
